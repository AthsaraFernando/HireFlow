...
class Interview
{
    ...
    {
        $stageSelect = $this->hasColumn('interview_stage')
            ? "i.interview_stage"
            : "'Screening' as interview_stage";

        $roleSelect = $this->hasColumn('interviewer_role')
            ? "i.interviewer_role"
            : "'HR Admin' as interviewer_role";
    ...
    {
        if (!$this->hasColumn('interview_stage')) {
            unset($data['interview_stage']);
        }

        if (!$this->hasColumn('interviewer_role')) {
            unset($data['interviewer_role']);
        }
    ...
    {
        if (array_key_exists($columnName, $this->columnSupportCache)) {
            return $this->columnSupportCache[$columnName];
        }
        ...
        $result = $this->get_row($query, [$this->table, $columnName]);
        $this->columnSupportCache[$columnName] = (int)($result['column_count'] ?? 0) > 0;
